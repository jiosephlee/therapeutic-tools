...
def _compute_pka(smiles: str) -> str:
    ...
    from .adme import _get_pka_data, _format_pka_section
    from . import metadata_cache
...
def _compute_ionization(smiles: str) -> str:
    ...
    from .adme import _compact_ionization
...
def _compute_logd(smiles: str) -> str:
    ...
    from .adme import _get_pka_data, _estimate_logd_from_pka
    from . import metadata_cache
...
def _compute_solubility(smiles: str) -> str:
    ...
    if cached and "minimol_solubility_log_mol_L" in cached:
        return f"Solubility: logS = {cached['minimol_solubility_log_mol_L']:.2f} log(mol/L)"
    try:
        from .legacy_tools.RDKit_tools import get_esol
        esol_result = get_esol(smiles)
        return f"Solubility (ESOL heuristic): {esol_result}"
    except Exception as e:
        return f"Solubility: Error - {e}"
