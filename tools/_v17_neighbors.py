"""Neighbor/analogue implementation for the v17 public tool wrapper."""
...
def _resolve_task_name(raw_task: str) -> Optional[str]:
    query = str(raw_task).lower()
    for task in TASKS:
        if query == task.lower():
            return task
    best: tuple[int, str] | None = None
    for task in TASKS:
        dist = _levenshtein(query, task.lower())
...
__all__ = ["K_NEIGHBORS", "get_neighbors"]
